...
//			Log.e(Util.LOG_TAG,"  HistoryListAdapter.getView  ");
		...
		// 発信元を設定
		TextView sourceNameTextView =(TextView) view.findViewById(R.id.history_list_source_name);
		sourceNameTextView.setText(item.getDisplayName());


		// 履歴の時間を設定
		TextView timeTextView = (TextView) view.findViewById(R.id.history_time);
		...
		// 着信か発信でマークを変える
		ImageView mark = (ImageView) view.findViewById(R.id.iv_in_or_out_mark);
		if (item.isInStatus()) {
			mark.setImageDrawable(activity.getResources().getDrawable(R.drawable.in_mark));
		} else if (item.isOutStatus()) {
			mark.setImageDrawable(activity.getResources().getDrawable(R.drawable.out_mark));
		} else {
