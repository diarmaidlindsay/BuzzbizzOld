...
//		Log.e(Util.LOG_TAG,"  TagFrameLayout.コンストラクタ  ");
		...
		// ビューをインフレート
		LayoutInflater.from(context.getApplicationContext()).inflate(R.layout.tag_flipper_item, this);


		// タブの設定が通常の場合は、
		if (MyFrameLayout.tabPosition == 0) {
			// 反転バージョンのダミータグを非表示にする
			findViewById(R.id.reverse_dummy_tab_layout).setVisibility(View.GONE);

			// 各ダミーボタンを取得
			dummyButtonGoToKeypad	= (Button)findViewById(R.id.dummy_button_to_key);
			dummyButtonGoToHold		= (Button)findViewById(R.id.dummy_button_to_hold);
			dummyButtonGoToIn		= (Button)findViewById(R.id.dummy_button_to_in);
			dummyButtonGoToEx		= (Button)findViewById(R.id.dummy_button_to_ex);
			dummyButtonGoToHST		= (Button)findViewById(R.id.dummy_button_to_hst);


		// タブの設定が反転している場合は、
		}else{
			// 通常のダミータグを非表示にする
			findViewById(R.id.dummy_tab_layout).setVisibility(View.GONE);

			// 各ダミーボタンを取得
			dummyButtonGoToKeypad	= (Button)findViewById(R.id.reverse_dummy_button_to_key);
			dummyButtonGoToHold		= (Button)findViewById(R.id.reverse_dummy_button_to_hold);
			dummyButtonGoToIn		= (Button)findViewById(R.id.reverse_dummy_button_to_in);
			dummyButtonGoToEx		= (Button)findViewById(R.id.reverse_dummy_button_to_ex);
			dummyButtonGoToHST		= (Button)findViewById(R.id.reverse_dummy_button_to_hst);
		}
	}
