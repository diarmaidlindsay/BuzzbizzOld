...
public class HoldListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(callerName == null || callerName.equals("null") || callerName.equals("")){
			...
			((TextView) view.findViewById(jp.pulseanddecibels.buzbiz.R.id.hold_list_caller_name)).setText(caller);
		// 取得に成功した場合は、
		}else{
			// リストの表示部分にお客様名を設定
			((TextView) view.findViewById(jp.pulseanddecibels.buzbiz.R.id.hold_list_caller_name)).setText(callerName);
		}
		...
		// リストの表示部分に対応者名を設定
		((TextView) view.findViewById(jp.pulseanddecibels.buzbiz.R.id.hold_list_responders_name)).setText("対応者 " + responders);
		...
		timeMin = time / 60;
		tTimeSec = time % 60;
		((TextView) view.findViewById(jp.pulseanddecibels.buzbiz.R.id.hold_list_timer)).setText(String.format("時間 %02d:%02d", timeMin, tTimeSec));
