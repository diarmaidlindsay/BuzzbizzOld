...
public class HoldScreen {
	...
	public HoldScreen(Context context, MyFrameLayout myFrameLayout) {
		...
		// ダミーのログイン画面のインフレート
		dummyLogin = ((Activity) context).getLayoutInflater().inflate(jp.pulseanddecibels.buzbiz.R.layout.login_screen, null);

		// 設定画面に遷移する為のボタンを設定
		Button buttonSetting = (Button)myFrameLayout.findViewById(jp.pulseanddecibels.buzbiz.R.id.setting);
        buttonSetting.setOnTouchListener(goToSetting);

		// 外線帳リストを設定
        holdList = (ListView) myFrameLayout.findViewById(jp.pulseanddecibels.buzbiz.R.id.hold_list);
                    ...
                    MainActivity.getHandler().post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (((Activity) context).isFinishing() == false) {
                                ...
                                HoldListAdapter baseAdapter = new HoldListAdapter(context, parcedArray, jp.pulseanddecibels.buzbiz.R.layout.hold_list_item);
...
//            if(MainService.CurentScreenState != MainService.HOLD){
            if(MainService.CurentScreenState != MainService.ScreenStates.HOLD){
                stopHoldScreenTimer();
                return;
            }
...
//				Log.e(Util.LOG_TAG,"  ACTION_UP  ");
                    ...
                    if(glTimer == null){
                        // ダミーのログイン画面の追加
                        ((FrameLayout)((Activity)context).findViewById(jp.pulseanddecibels.buzbiz.R.id.gl_plus_layout)).addView(dummyLogin, 1);
